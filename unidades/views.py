...
def cadastro_filial(request):
    if request.method == "GET":
        ...
        if not request.user.is_authenticated:
            messages.add_message(request, constants.ERROR, "Para cadastrar filial, tem que estar logado.")
            return redirect("/unidades/cadastro_filial")
    
        if not nome:
            messages.add_message(request, constants.ERROR, "Nome da filial é obrigatório")
            return redirect("/unidades/cadastro_filial")
        try:
            dados_filial = Filial(nome=nome, cnpj=cnpj, rua=rua, bairro=bairro, numero=numero, cidade=cidade, foto=foto, user=request.user)
            dados_filial.save()
            messages.add_message(request, constants.SUCCESS, "Cadastro da Filial realizado com sucesso.")
            return redirect("/unidades/cadastro_filial")
        except:
            messages.add_message(request, constants.ERROR, "Erro ao salvar esta Filial.")
            return redirect("/unidades/cadastro_filial")
...
def home(request):
    if request.method == "GET":
        ...
        saldos = Saldo.objects.filter(
            data__range=[datetime.now().date() - timedelta(days=1), datetime.now().date() + timedelta(days=1),]).order_by("filial__id", "data")
        ...
        context = {"data_atual":data_atual, "saldos": saldos, "filiais": filiais, "tg": tg, "tm": tm, "soma": soma,}
        ...
        if termasa == "": termasa = "0"
        if tergrasa == "": tergrasa = "0"
        soma = int(tergrasa) + int(termasa)
        try:
            saldos = Saldo.objects.filter(filial=filial, data=data)
        except:
            messages.add_message(request, constants.ERROR, "Confira as informações digitadas.")
            return redirect("/unidades/home")

        if not saldos:
            try:
                saldo = Saldo(data=data, tergrasa=tergrasa, termasa=termasa, total=str(soma), filial_id=filial,)
                saldo.save()
...
def editar_saldo(request, id):
    if request.method == "GET":
        ...
        context = {"idd": id, "saldos": saldos, "saldo_editado": saldo_editado, "tg": tg, "tm": tm, "soma": soma}
